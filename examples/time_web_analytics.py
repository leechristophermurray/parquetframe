...
web_traffic = pd.DataFrame({
    ...
    'user_id': np.random.choice(range(1, 1001), size=num_requests),  # 1000 unique users
    'page_view_duration': np.abs(np.random.gamma(2, 30, size=num_requests)),  # seconds
    'endpoint': np.random.choice(['/home', '/products', '/api/data', '/checkout', '/profile'], 
                                 size=num_requests,
                                 p=[0.4, 0.25, 0.15, 0.1, 0.1]),
    'status_code': np.random.choice([200, 404, 500], size=num_requests, p=[0.95, 0.04, 0.01]),
})
...
print(f"Time range: {web_traffic['timestamp'].min()} to {web_traffic['timestamp'].max()}")
print(f"Unique users: {web_traffic['user_id'].nunique():,}")
print()

# Import ParquetFrame TIME operations
from parquetframe._rustic import time_resample_sum, time_rolling_mean
...
web_traffic_sorted['time_diff'] = web_traffic_sorted.groupby('user_id')['timestamp'].diff()
web_traffic_sorted['new_session'] = (web_traffic_sorted['time_diff'] > timedelta(minutes=30)) | \
                                     (web_traffic_sorted['time_diff'].isna())
web_traffic_sorted['session_id'] = web_traffic_sorted.groupby('user_id')['new_session'].cumsum()

# Session statistics
sessions = web_traffic_sorted.groupby(['user_id', 'session_id']).agg({
    'timestamp': ['min', 'max', 'count'],
    'page_view_duration': 'sum'
})

sessions.columns = ['session_start', 'session_end', 'page_views', 'total_duration']
sessions['session_length'] = (sessions['session_end'] - sessions['session_start']).dt.total_seconds() / 60
...
error_by_endpoint = web_traffic.groupby(['endpoint', 'status_code']).size().unstack(fill_value=0)
error_by_endpoint['error_rate'] = (error_by_endpoint.get(404, 0) + error_by_endpoint.get(500, 0)) / \
...
for endpoint in error_by_endpoint.index:
    ...
    errors = error_by_endpoint.loc[endpoint].get(404, 0) + error_by_endpoint.loc[endpoint].get(500, 0)
...
user_activity = web_traffic.groupby('user_id').agg({
    'timestamp': ['min', 'max', 'count'],
    'page_view_duration': 'mean'
})

user_activity.columns = ['first_seen', 'last_seen', 'total_requests', 'avg_duration']
user_activity['days_active'] = (user_activity['last_seen'] - user_activity['first_seen']).dt.total_seconds() / 86400
...
for segment in ['Low', 'Medium', 'High']:
    ...
    avg_requests = user_activity[user_activity['segment']  == segment]['total_requests'].mean()
    print(f"  {segment:<10} {count:>6} users  (avg {avg_requests:>6.1f} requests)")

# Top users
print("\nTop 5 Most Active Users:")
top_users = user_activity.nlargest(5, 'total_requests')[['total_requests', 'avg_duration']]
for user_id, row in top_users.iterrows():
    print(f"  User {user_id:>4}: {row['total_requests']:>4.0f} requests, avg {row['avg_duration']:>5.1f}s per page")
...
print("=" * 80)
print("\nKey Insights:")
print(f"• Processed {len(web_traffic):,} requests from {web_traffic['user_id'].nunique():,} users")
print(f"• Peak hour: {peak_hour}:00 with {peak_requests:,} requests")
print(f"• Average session: {sessions['page_views'].mean():.1f} pages, {sessions['session_length'].mean():.1f} minutes")
