...
class KanbanAnalytics:
    ...
    def get_completion_metrics(self, board_id: str, user_id: str) -> pd.DataFrame:
        ...
        # Daily task creation
        daily_created = tasks_df.ts.resample('1D', agg='count')['task_id']
    ...
    def analyze_team_velocity(self, board_id: str, user_id: str) -> dict:
            ...
            for task in tasks:
                if task.status == 'done' and task.updated_at:
                    completion_data.append({
                        'date': pd.to_datetime(task.updated_at).date(),
                        'count': 1
                    })
        ...
        print(f"Current 7-day average: {daily['velocity_sma_7'].iloc[-1]:.2f} tasks/day")
        print(f"Trend (EMA): {daily['velocity_trend'].iloc[-1]:.2f} tasks/day")
        
        return {
            'average_velocity': daily['count'].mean(),
            'recent_velocity': daily['velocity_sma_7'].iloc[-1] if len(daily) >= 7 else daily['count'].mean(),
            'trend': 'increasing' if daily['velocity_trend'].iloc[-1] > daily['velocity_trend'].iloc[-7] if len(daily) >= 7 else 'stable' else 'stable'
        }
    ...
    def generate_burndown_chart(self, board_id: str, user_id: str) -> pd.DataFrame:
        ...
        for date in dates:
            remaining = sum(
                ...
                if task.created_at <= date and 
                   (task.status != 'done' or (task.updated_at and task.updated_at > date))
            )
...
def demo_enhanced_kanban():
    ...
    completion_metrics = analytics.get_completion_metrics(board.board_id, alice.user_id)
    
    # 2. Financial-style velocity analysis
    velocity = analytics.analyze_team_velocity(board.board_id, alice.user_id)
    
    # 3. Time series burndown
    burndown = analytics.generate_burndown_chart(board.board_id, alice.user_id)
