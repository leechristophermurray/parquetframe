...
try:
    ...
    from parquetframe.benchmark import PerformanceBenchmark, run_comprehensive_benchmark
except ImportError:
...
def create_sample_datasets():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        large_data = pd.DataFrame({
            ...
            'category': np.random.choice(['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'], 200000),
...
def demonstrate_intelligent_backend_switching(datasets):
    ...
    for name, file_path in datasets.items():
        ...
        pf_pandas = ParquetFrame.read(str(file_path), islazy=False)
        pf_dask = ParquetFrame.read(str(file_path), islazy=True)
        print(f"  Explicit:      pandas (forced), Dask (forced)")
...
def demonstrate_performance_benchmarking(datasets, temp_path):
    ...
    for result in read_results:
        ...
        print(f"  {status} {result.operation} ({result.backend}): "
...
def demonstrate_memory_aware_processing(datasets):
    ...
    for name, file_path in datasets.items():
        ...
        print(f"  Memory estimate: {ParquetFrame._estimate_memory_usage(file_path):.1f} MB")
...
def demonstrate_advanced_usage(datasets):
    ...
    for name, file_path in datasets.items():
        ...
        try:
            result = (pf
                     .query("value > 0")  # Filter positive values
            ...
            if pf.islazy:
                ...
                small_result = pf.head(10).to_pandas()
            else:
...
def save_benchmark_results(benchmark, temp_path):
    ...
    # Compile results
    all_results = {
        'read_operations': [r.__dict__ for r in benchmark.results if 'Read' in r.operation],
        'data_operations': [r.__dict__ for r in benchmark.results if 'Read' not in r.operation],
        'summary': {
    ...
    print(f"  • Success rate: {all_results['summary']['successful_benchmarks']/all_results['summary']['total_benchmarks']*100:.1f}%")
