...
sensor_data = pd.DataFrame({
    ...
    'temperature': 20 + 5 * np.sin(2 * np.pi * np.arange(len(timestamps)) / (24 * 60)) + 
                   np.random.randn(len(timestamps)) * 0.5,
    # Humidity sensor (%) - inverse correlation with temp
    'humidity': 60 - 10 * np.sin(2 * np.pi * np.arange(len(timestamps)) / (24 * 60)) +
...
print(f"Reduction factor:          {len(sensor_data) / len(resampled_15min['values']):>8.1f}x")
...
print(f"Reduction factor:          {len(sensor_data) / len(resampled_hourly['values']):>8.1f}x")
...
sensor_data['z_score'] = (sensor_data['temperature'] - sensor_data['rolling_mean']) / sensor_data['rolling_std']
...
anomalies = sensor_data[sensor_data['is_anomaly']][['timestamp', 'temperature', 'rolling_mean', 'z_score']].head(5)
for _, row in anomalies.iterrows():
    print(f"  {row['timestamp']}  {row['temperature']:>6.2f}°C (expected: {row['rolling_mean']:>6.2f}°C, z={row['z_score']:>5.2f})")
...
for hour in [0, 6, 12, 18]:
    ...
    print(f"{hour:>4}  {stats['mean']:>6.2f}  {stats['std']:>6.2f}  {stats['min']:>6.2f}  {stats['max']:>6.2f}")
...
print(f"• Temperature and humidity show {abs(corr_temp_humidity):.2f} correlation (inverse)")
