...
class BPETokenizer:
    ...
    def train(self, texts: List[str], max_merges: int | None = None):
        ...
        logger.info(f"Trained BPE with {len(self.vocab)} tokens and {len(self.merges)} merges")
    ...
    def decode(self, token_ids: List[int]) -> str:
        ...
        for token_id in token_ids:
            if token_id in [self.vocab[self.pad_token], self.vocab[self.bos_token], self.vocab[self.eos_token]]:
                continue
