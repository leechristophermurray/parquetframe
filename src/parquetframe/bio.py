...
class BioAccessor:
        ...
        if self._is_lazy:
            ...
            def cluster_partition(df_part):
                ...
                if len(df_part) == 0:
                    return df_part
                try:
                    return bf.cluster(df_part, by=by, min_dist=min_dist, 
            ...
            try:
                result_df = bf.cluster(self._df, by=by, min_dist=min_dist,
                                     max_dist=max_dist, **kwargs)
        ...
        if self._is_lazy or other.islazy:
            if broadcast:
                ...
                def overlap_partition(df_part, small_df):
                    ...
                    try:
                        if large_is_self:
                            return bf.overlap(df_part, small_df, how=how, on=on, **kwargs)
                        else:
                            return bf.overlap(small_df, df_part, how=how, on=on, **kwargs)
                    except Exception as e:
    ...
    def complement(
        self,
        genome: Optional[Union[pd.DataFrame, dict]] = None,
        **kwargs: Any
    ) -> "ParquetFrame":
        """
