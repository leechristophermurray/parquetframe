...
class ParquetFrame:
    ...
    @classmethod
    def read(cls, file: Union[str, Path], threshold_mb: Optional[float] = None, 
        ...
        if use_dask:
            ...
            print(f"Reading '{file_path}' as Dask DataFrame (size: {file_size_mb:.2f} MB)")
        else:
            df = pd.read_parquet(file_path, **kwargs)
            print(f"Reading '{file_path}' as pandas DataFrame (size: {file_size_mb:.2f} MB)")
    ...
    def _resolve_file_path(file: Union[str, Path]) -> Path:
        ...
        raise FileNotFoundError(f"No parquet file found for '{file}' (tried .parquet, .pqt)")
