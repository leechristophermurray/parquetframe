...
class WorkflowStep(ABC):
    ...
    def interpolate_variables(self, value: Any, context: WorkflowContext) -> Any:
        ...
        for var_name, var_value in context.variables.items():
            ...
            result = result.replace(f"${var_name}", str(var_value))  # Support both ${var} and $var
...
class FilterStep(WorkflowStep):
    ...
    def execute(self, context: WorkflowContext) -> ParquetFrame:
        ...
        if pf is None:
            raise WorkflowExecutionError(f"Step '{self.name}': Dataset '{input_name}' not found")
...
class GroupByStep(WorkflowStep):
    ...
    def execute(self, context: WorkflowContext) -> ParquetFrame:
        ...
        if isinstance(agg_config, str):
            ...
            raise WorkflowExecutionError(f"Step '{self.name}': Invalid aggregation configuration")
        ...
        for transform in transforms:
            if 'column' in transform and 'operation' in transform:
                ...
                if operation == 'rename':
                    ...
                    warnings.warn(f"Transform operation '{operation}' not fully supported yet")
...
class WorkflowEngine:
    ...
    def load_workflow(self, workflow_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        if not YAML_AVAILABLE:
            raise WorkflowError("PyYAML is required for workflow support. Install with: pip install pyyaml")
    ...
    def execute_workflow(
        ...
        with Progress(
            ...
            for i, step_config in enumerate(steps):
                ...
                if self.verbose:
                    progress.update(main_task, description=f"Executing {step_name} ({step_type})")
        ...
        if self.verbose and self.console:
            ...
            if context.datasets:
                ...
                for name, dataset in context.datasets.items():
                    ...
                    shape = str(dataset.shape) if hasattr(dataset, 'shape') else "Unknown"
    ...
    def run_workflow_file(
        ...
        # Execute workflow
        return self.execute_workflow(workflow, working_dir=working_dir, variables=variables)
