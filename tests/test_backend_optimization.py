...
class TestIntelligentBackendSwitching:
    ...
    def test_estimate_memory_usage_with_pyarrow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            test_data = pd.DataFrame(
                {"A": range(1000), "B": ["test"] * 1000, "C": [1.5] * 1000}
            )
    ...
    def test_should_use_dask_explicit_islazy_true(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = ParquetFrame._should_use_dask(
                test_file, threshold_mb=10, islazy=True
            )
    ...
    def test_should_use_dask_explicit_islazy_false(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = ParquetFrame._should_use_dask(
                test_file, threshold_mb=10, islazy=False
            )
    ...
    def test_should_use_dask_basic_threshold_over(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = ParquetFrame._should_use_dask(
                test_file, threshold_mb=10, islazy=None
            )
    ...
    def test_should_use_dask_memory_pressure(
        self, mock_get_memory, mock_estimate_memory
    ):
    ...
    @patch.object(ParquetFrame, "_estimate_memory_usage")
    @patch.object(ParquetFrame, "_get_system_memory")
    def test_should_use_dask_low_memory_pressure(
        self, mock_get_memory, mock_estimate_memory
    ):
    ...
    @patch.object(ParquetFrame, "_estimate_memory_usage")
    @patch.object(ParquetFrame, "_get_system_memory")
    def test_should_use_dask_many_row_groups(
        self, mock_get_memory, mock_estimate_memory, mock_parquet_file
    ):
    ...
    @patch.object(ParquetFrame, "_estimate_memory_usage")
    @patch.object(ParquetFrame, "_get_system_memory")
    def test_should_use_dask_few_row_groups(
        self, mock_get_memory, mock_estimate_memory, mock_parquet_file
    ):
    ...
    @patch.object(ParquetFrame, "_estimate_memory_usage")
    @patch.object(ParquetFrame, "_get_system_memory")
    def test_should_use_dask_metadata_error(
        self, mock_get_memory, mock_estimate_memory, mock_parquet_file
    ):
...
class TestBackendSwitchingIntegration:
    ...
    def test_read_with_intelligent_switching(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            # Create a test parquet file
            test_data = pd.DataFrame({"A": range(100), "B": ["test"] * 100})
            test_file = Path(temp_dir) / "test.parquet"
            test_data.to_parquet(test_file)
    ...
    def test_real_file_backend_selection(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            large_pf = ParquetFrame.read(
                str(large_file), threshold_mb=1
            )  # Low threshold
