...
try:
    from parquetframe.workflows import (
        WorkflowEngine, WorkflowContext, WorkflowError, WorkflowValidationError,
        WorkflowExecutionError, ReadStep, FilterStep, SelectStep, GroupByStep,
        SaveStep, TransformStep, STEP_REGISTRY, create_example_workflow
    )
...
@pytest.mark.skipif(not WORKFLOW_AVAILABLE, reason="Workflow functionality not available")
...
class TestWorkflowSteps:
    ...
    def test_transform_step_validation(self):
        ...
        # Valid configuration
        step = TransformStep("test", {"transforms": [{"column": "a", "operation": "rename", "to": "b"}]})
        errors = step.validate()
        assert errors == []
...
class TestStepRegistry:
    def test_step_registry_completeness(self):
        ...
        expected_steps = {
            'read', 'filter', 'select', 'groupby', 'save', 'transform'
        }
