...
class TestCLIRun:
    ...
    def test_run_command_with_query(self, cli_runner, sample_parquet):
        ...
        result = cli_runner.invoke(
            main, ["run", str(sample_parquet), "--query", "age > 30"]
        )
    ...
    def test_run_command_with_columns(self, cli_runner, sample_parquet):
        ...
        result = cli_runner.invoke(
            main, ["run", str(sample_parquet), "--columns", "name,age"]
        )
    ...
    def test_run_command_with_head(self, cli_runner, sample_parquet):
        ...
        result = cli_runner.invoke(main, ["run", str(sample_parquet), "--head", "3"])
    ...
    def test_run_command_with_describe(self, cli_runner, sample_parquet):
        ...
        result = cli_runner.invoke(main, ["run", str(sample_parquet), "--describe"])
    ...
    def test_run_command_with_output(self, cli_runner, sample_parquet, tmp_path):
        ...
        result = cli_runner.invoke(
            main, ["run", str(sample_parquet), "--output", str(output_file)]
        )

        assert result.exit_code == 0
        # Check for either CLI message or core save message
        assert (
            f"Saving to: {output_file}" in result.output
            or f"DataFrame saved to '{output_file}'." in result.output
        )
        assert output_file.exists()

    def test_run_command_with_script_generation(
        self, cli_runner, sample_parquet, tmp_path
    ):
    ...
    def test_run_command_force_backends(self, cli_runner, sample_parquet):
        ...
        # Test force pandas
        result = cli_runner.invoke(main, ["run", str(sample_parquet), "--force-pandas"])
        assert result.exit_code == 0
        assert "pandas DataFrame" in result.output

        # Test force dask
        result = cli_runner.invoke(main, ["run", str(sample_parquet), "--force-dask"])
        assert result.exit_code == 0
    ...
    def test_run_command_conflicting_backends(self, cli_runner, sample_parquet):
        ...
        result = cli_runner.invoke(
            main, ["run", str(sample_parquet), "--force-pandas", "--force-dask"]
        )
...
class TestCLIErrorHandling:
    ...
    def test_run_invalid_query(self, cli_runner, sample_parquet):
        ...
        result = cli_runner.invoke(
            main, ["run", str(sample_parquet), "--query", "invalid_column > 30"]
        )
    ...
    def test_run_invalid_columns(self, cli_runner, sample_parquet):
        ...
        result = cli_runner.invoke(
            main, ["run", str(sample_parquet), "--columns", "nonexistent_column"]
        )
    ...
    def test_different_file_sizes(self, cli_runner, tmp_path):
        ...
        result = cli_runner.invoke(
            ...
            [
                ...
                str(tiny_file),
                "--threshold",
                "0.001",  # Very small threshold to force Dask
            ],
        )
        assert result.exit_code == 0
