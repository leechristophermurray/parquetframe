...
from parquetframe.sql import query_dataframes, validate_sql_query, SQLError, explain_query
...
class TestSQLModule:
    ...
    @pytest.mark.skipif(
        condition=lambda: not hasattr(pytest, 'importorskip') or not pytest.importorskip('duckdb'),
    ...
    def test_sql_with_dask_dataframes(self, sample_data):
        ...
        with pytest.warns(UserWarning, match="SQL queries on Dask DataFrames"):
            result = query_dataframes(ddf1, "SELECT * FROM df WHERE age > 25", {'other': ddf2})
...
class TestSQLIntegration:
    ...
    def test_complex_aggregation(self):
        ...
        sales_df = pd.DataFrame({
            ...
            'region': ['North', 'South', 'North', 'South', 'North', 'South', 'East', 'West'],
        ...
        assert len(result) >= 1
        assert all(col in result.columns for col in ['product_name', 'category', 'total_revenue'])
