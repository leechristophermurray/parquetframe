    ...
    async def test_magic_sql_with_permissions(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("parquetframe.interactive.INTERACTIVE_AVAILABLE", True):
                ...
                # Grant permission
                session._handle_permissions_command("grant user:alice viewer table:users")
                
                # Check permission
                session._handle_permissions_command("check user:alice viewer table:users")
                
                # Execute SQL query
                await session._handle_query("SELECT * FROM users WHERE department = 'Engineering'")
...
class TestEntityForeignKeyIntegration:
    ...
    def test_entity_relationship_validation_workflow(self):
        ...
        entity_store.get_entity.side_effect = lambda name: users_df if name == "User" else tasks_df
...
class TestPerformanceBenchmarks:
    ...
    def test_graph_utils_performance(self):
        ...
        from parquetframe.graph.algo.utils import validate_sources, create_result_dataframe
        import time
    ...
    def test_foreign_key_validation_performance(self):
        ...
        entity_store.get_entity.return_value = pd.DataFrame({
            "id": range(10000),
            "name": [f"Entity_{i}" for i in range(10000)]
        })
        manager.set_entity_store(entity_store)
